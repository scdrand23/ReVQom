...
from revqom.models.comm_modules.where2comm import Communication
from revqom.models.fuse_modules.fuse_utils import regroup as regroup_utils
...
class Where2comm(nn.Module):
    def __init__(self, args):
        ...
        self.max_cav = args.get('max_cav', 5)  # Maximum number of agents, default to 5
    ...
    def regroup(self, x, record_len, return_mask=False):
        # Use the existing regroup from fuse_utils
        if return_mask:
            regroup_feature, mask = regroup_utils(x, record_len, self.max_cav)
            return regroup_feature, mask
        else:
            # For backward compatibility, just return the split features without padding
