...
def load_anchor_free_params(param, opt=None):
    """
    Load and modify parameters for TransFusion-based
    
    Parameters
    ----------
    param : dict
        Original parameter dictionary loaded from yaml
    ...
    """
    # Calculate grid size based on lidar range and voxel size
    cav_lidar_range = param['preprocess']['cav_lidar_range']
    voxel_size = param['preprocess']['args']['voxel_size']
    
    grid_size = (np.array(cav_lidar_range[3:6]) - np.array(cav_lidar_range[0:3])) / \
                np.array(voxel_size)
    grid_size = np.round(grid_size).astype(np.int64)
        # grid_size = np.round(grid_size).astype(np.int64)
    if 'him' in param['model']['args']:
        param['model']['args']['him']['head_cfg_args']['grid_size'] = grid_size
    param['model']['args']['grid_size'] = grid_size
    param['loss']['args']['grid_size'] = grid_size
    param['model']['args']['dense_head']['grid_size'] = grid_size
    # param['model']['args']['point_pillar_scatter']['grid_size'] = grid_size if 'point_pillar_scatter' in param['model']['args'] else None
